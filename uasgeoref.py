...
#An srtm .hgt for the region can be placed in the directory \DEM\SRTM3 if not in N. America.
...
#gpx files: \GPX\Done\date, world files: \World_Files\date, prj files \PRJs\date, aux.xml files \AUXXMLs\date.
#Copy the world files, prj, aux.xml and geotagged images into a single folder outside of this program.
...
while os.path.exists(originalspath + '/' + name):
    ...
    num = num + 1
DEM_path = os.path.join(path,'DEM\SRTM3')
...
jpwpath = os.path.join(path, 'World_Files')
save_path_jpw = os.path.join(path,'World_Files', name)
